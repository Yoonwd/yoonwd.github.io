...
/******/ 	// eslint-disable-next-line no-unused-vars
/******/ 	var hotCurrentHash = "65810bf6e04c86c4bd1e";
...
// module
exports.push([module.i, ".src-components-style__appContainer___2nRi7 {\n    width: 100%;\n    height: 100%;\n}\n\n\n.src-components-style__App___1PmRE {\n\ttext-align: center;\n  }\n  \n  .src-components-style__App-logo___6cO6i {\n\tanimation: src-components-style__App-logo-spin___KLKil infinite 20s linear;\n\theight: 80px;\n  }\n  \n  .src-components-style__App-header___2PGxJ {\n\tbackground-color: #222;\n\theight: 150px;\n\tpadding: 20px;\n\tcolor: white;\n  }\n  \n  .src-components-style__App-title___210MM {\n\tfont-size: 1.5em;\n  }\n  \n  .src-components-style__App-intro___3QpDj {\n\tfont-size: large;\n  }\n  \n  @keyframes src-components-style__App-logo-spin___KLKil {\n\tfrom { transform: rotate(0deg); }\n\tto { transform: rotate(360deg); }\n  }\n  \n  \n  /* Custom Report table */\n  /* hidding user if columns */\n  .src-components-style__report__preview-container___xxYD6 .src-components-style__page__table___1YswC .src-components-style__header___3oYHb .src-components-style__col___2nrTf.src-components-style__userId___3aWYR,\n  .src-components-style__report__preview-container___xxYD6 .src-components-style__page__table___1YswC .src-components-style__row___372xk .src-components-style__col___2nrTf.src-components-style__userId___3aWYR {\n\tvisibility: hidden;\n\tdisplay: none;\n  }\n  ", ""]);

// exports
exports.locals = {
	"appContainer": "src-components-style__appContainer___2nRi7",
	"App": "src-components-style__App___1PmRE",
	"App-logo": "src-components-style__App-logo___6cO6i",
	"App-logo-spin": "src-components-style__App-logo-spin___KLKil",
	"App-header": "src-components-style__App-header___2PGxJ",
	"App-title": "src-components-style__App-title___210MM",
	"App-intro": "src-components-style__App-intro___3QpDj",
	"report__preview-container": "src-components-style__report__preview-container___xxYD6",
	"page__table": "src-components-style__page__table___1YswC",
	"header": "src-components-style__header___3oYHb",
	"col": "src-components-style__col___2nrTf",
	"userId": "src-components-style__userId___3aWYR",
	"row": "src-components-style__row___372xk"
};
...
exports.default = {
    RECEIVED_BUBBLE_DATA: 'received bubble data'
};
...
var _ActionCreator = __webpack_require__(20);
...
exports.default = {
    requestBubbleData: function requestBubbleData(keyword) {
        fetch(api + '/rels?suid=chajazoom', option({
            requestInfos: [{
                keyword: '손흥민'
            }]
        })).then(function (res) {
            ...
            if (res.code === 'SUCCESS') {
                _ActionCreator2.default.receivedBubbleData(res.data);
            } else {
...
/* concated harmony reexport */__webpack_require__.d(__webpack_exports__, "stackOffsetDiverging", function() { return offset_diverging; });
...
var BubbleChart = function (_Component) {
	...
	function BubbleChart(props) {
		...
		_this.bubbleClick = _this.bubbleClick.bind(_this);
		_this.state = {
			selected: []
		};
		_this.tween = true;
	...
	_createClass(BubbleChart, [{
		...
		value: function componentDidUpdate(prevProps) {
			...
			var w = this.svg.getBoundingClientRect().width;
			var h = this.svg.getBoundingClientRect().height;
			if (width !== w || height !== h) {
				this.renderChart();
			}
		}
		...
		key: 'bubbleClick',
		value: function bubbleClick(d) {
			var _this2 = this;

			var find = function find(id) {
				return _this2.state.selected.indexOf(id);
			};
			if (find(d.id) > -1) {
				this.setState({ selected: this.state.selected.filter(function (v) {
						return v !== d.id;
					}) });
			} else {
				this.setState({ selected: [].concat(_toConsumableArray(this.state.selected), [d.id]) });
			}
			window.jQuery(document).trigger('post.bubble.keywords', [this.state.selected]);
			var nodes = d3.selectAll('.node');
			nodes.attr('class', function (d) {
				return find(d.id) > -1 ? !d.empty && d.max ? 'node max on' : 'node on' : !d.empty && d.max ? 'node max' : 'node';
			}).transition().duration(1500).ease(d3.easeElasticOut).attr('transform', function (d) {
				var ratio = find(d.id) > -1 ? 1.1 : 1;
				return 'translate(' + _this2.xscaleMapper(d.x) + ',' + d.y + ') scale(' + ratio + ',' + ratio + ')';
			});

			this.props.bubbleClickFun(d);
		}
	}, {
		key: 'render',
		...
		value: function renderChart() {
			...
			var bubblesHeight = height;
			var legendWidth = width - bubblesWidth;
			var color = d3.scaleOrdinal(d3.schemeCategory20c);

			var pack = d3.pack().size([height, height]).padding(10);
			...
			var root = d3.hierarchy({ children: data }).sum(function (d) {
				...
				d.max = d.data.value == max;
				d.empty = d.data.empty;
			});

			var nodes = pack(root).leaves();
			this.renderBubbles(bubblesWidth, bubblesHeight, nodes, color);
			if (showLegend) {
				this.renderLegend(legendWidth, bubblesHeight, bubblesWidth, nodes, color);
			}
		}
	}, {
		key: 'renderBubbles',
		value: function renderBubbles(width, height, nodes, color) {
			var _this3 = this;
			...
			var bubbleChart = d3.select(this.svg).append('g').attr('class', 'bubble-chart').attr('transform', function (d) {
				return 'translate(' + width * graph.offsetX + ',' + height * graph.offsetY + ')';
			});

			var node = bubbleChart.selectAll('.node').data(nodes).enter().append('g').attr('class', function (d) {
				return !d.empty && d.max ? 'node max' : 'node';
			}).attr('transform', function (d) {
				return 'translate(' + width * 0.5 * graph.zoom + ',' + height * 0.5 * graph.zoom + ') scale(0,0)';
			}).on('click', function (d) {
				d3.select(this).bringElementAsTopLayer();
				!d.empty && chart.bubbleClick(d);
			});

			if (this.tween) {
				node.transition().delay(function (d, i) {
					return 30 * i;
				}).duration(1500).ease(d3.easeElasticOut).attr('transform', function (d) {
					return 'translate(' + _this3.xscaleMapper(d.x) + ',' + d.y + ') scale(1,1)';
				});
			} else {
			...
			defs.append('pattern').attr('id', function (d) {
				return d.id;
			}).attr('width', '100%').attr('height', '100%').attr('patternContentUnits', 'objectBoundingBox').append('image').attr('width', 1).attr('height', 1).attr('perserveAspectRatio', 'none').attr('xmlns:xlink', 'http://www.w3.org/1999/xlink').attr('xlink:href', function (d) {
				return d.empty ? '' : d.data.image;
			});

			node.append('circle').attr('class', 'circle').attr('id', function (d) {
				return d.id;
			}).attr('r', function (d) {
				return d.r;
			...
			node.append('circle').attr('class', 'circle_cover').attr('r', function (d) {
				return d.r + 1;
			});

			node.append("foreignObject").attr("x", function (d) {
				return -d.r;
			}).attr("y", function (d) {
				return -d.r;
				...
				return d.r * 2;
			}).append('xhtml:div').attr('class', 'text-box').append('div').attr('class', 'label-text').html(function (d) {
				return d.label;
			}).append('div').attr('class', 'value-text').html(function (d) {
				return d.empty ? '' : d.value;
			});
...
exports.default = {
    receivedBubbleData: function receivedBubbleData(data) {
        _AppDispatcher2.default.dispatch({
            type: _Actions2.default.RECEIVED_BUBBLE_DATA,
            data: data
        });
    },
...
var issueKeywordList = [
// 강제로 표시하고 싶은 내용에 @@@@를 추가해 주세요. 카운트가 표시 되지 않습니다.
{
	"id": "17d9b695-8983-4d5e-87ee-d0c7dc405c17",
	"imageUrl": 'http://localhost:9999/img/issue_img_01.png',
	"issueKeyword": "@@@@이것은 공백 채우기용 버블 입니다1",
	...
	"issueKeyword": "@@@@이것은 공백 채우기용 버블 입니다2",
	...
	"issueKeyword": "@@@@이것은 공백 채우기용 버블 입니다3",
	"keywordCategory": 'cate2',
	"keywordDate": null,
	"count": 17
}, {
...
var App = function (_Component) {
	...
	_createClass(App, [{
		...
		value: function componentDidMount() {
			...
			this.registID = _AppDispatcher2.default.register(this.onDispatching.bind(this));
			_request2.default.requestBubbleData(this.props.keyword);
		}
	}, {
		key: 'componentWillUnmount',
		value: function componentWillUnmount() {
			_AppDispatcher2.default.unregister(this.registID);
		...
		value: function shouldComponentUpdate(nextProps, nextState) {
			return nextState.data != null && nextState.width > 0 && nextState.height > 0;
		}
	}, {
		key: 'onDispatching',
		...
		value: function getKeywords() {
			return this.state.data && this.state.data.relsKeywordList || [];
		}
	}, {
		key: 'bubbleClick',
		...
		key: 'initialize',
		value: function initialize(e, keyword) {
			_AppDispatcher2.default.register(this.onDispatching.bind(this));
			_request2.default.requestBubbleData(keyword);
		}
	}, {
		key: 'render',
		value: function render() {
			var data = issueKeywordList.map(function (d) {
				return {
					label: d.issueKeyword.replace('@@@@', '') || '없음',
					value: d.count || Math.floor(Math.random() * 10) + 1,
					empty: d.issueKeyword.indexOf('@@@@') != -1,
					id: d.id || d.issueKeyword,
					image: './img/issue_img_02.png'
				};
			});
			...
			var chart = _react2.default.createElement(_chart2.default, {
				graph: {
					zoom: 1,
					offsetX: 0,
					offsetY: 0
			...
			return _react2.default.createElement(
				...
				width > 0 && height > 0 && data.length > 0 && chart || null
			);
		}
	}]);
...
*/
var aa=__webpack_require__(0),ba=__webpack_require__(1),m=__webpack_require__(30),p=__webpack_require__(4),v=__webpack_require__(9),da=__webpack_require__(29),ea=__webpack_require__(28),fa=__webpack_require__(27),ha=__webpack_require__(10);
...
var $ = window.jQuery;
var container = document.getElementById('container_bubble_chart');
console.log(container);
$(document).on('start.bubble.chart', function (e, keyword) {
	$(container).show();
	_reactDom2.default.render(_react2.default.createElement(_App2.default, { keyword: keyword }), container);
});

$(document).on('stop.bubble.chart', function (e) {
	_reactDom2.default.render(null, container);
	$(container).hide();
});
